...
/* AUTHORS : C. Bouillaguet, parallelised by T. Chenaille
...
#define COPPER
#define NORMAL    /* Slowest to fastest : CHALLENGE, NORMAL, MEDIUM, and FAST (for debugging) */
...
int main(int argc, char *argv[])
{
    ...
    while (convergence == 0) {
        ...
        /* Mise à jour des cellules. On traite les plans zx, pour des valeurs croissantes de y. */
        for (int k = start_layer; k < (end_layer+1); k++) {   // y
            int local_index = (k - start_layer) * n * m;
        ...
        if (size > 1){
        if (rank == 0) {
            // Pour le premier processus, envoi à rank+1 et réception de rank+1
            for (int f = 0; f < (n * m); f++) {
                send_buffer_toNextProcess[f] = local_R[end_layer * n * m + f];
            }
            MPI_Sendrecv(send_buffer_toNextProcess, n * m, MPI_DOUBLE, 1, 0,
                         recv_buffer_fromNextProcess, n * m, MPI_DOUBLE, 1, 0,
                         MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }
        else if (rank == (size-1)) {
            // Pour le dernier processus, envoi à rank-1 et réception de rank-1
            for (int f = 0; f < (n * m); f++) {
                send_buffer_toPrevProcess[f] = local_R[f];
            }
            MPI_Sendrecv(send_buffer_toPrevProcess, n * m, MPI_DOUBLE, rank-1, 0,
                         recv_buffer_fromPrevProcess, n * m, MPI_DOUBLE, rank-1, 0,
                         MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }
        else {
            // Pour les processus intermédiaires, envoi et réception dans les deux directions
            for (int f = 0; f < (n * m); f++) {
                send_buffer_toPrevProcess[f] = local_R[f];
            ...
            MPI_Sendrecv(send_buffer_toPrevProcess, n * m, MPI_DOUBLE, rank-1, 0,
                         recv_buffer_fromPrevProcess, n * m, MPI_DOUBLE, rank-1, 0,
                         MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            MPI_Sendrecv(send_buffer_toNextProcess, n * m, MPI_DOUBLE, rank+1, 0,
                         recv_buffer_fromNextProcess, n * m, MPI_DOUBLE, rank+1, 0,
                         MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }}
            ...
            //Si size > 1, on ne fait pas la racine sur le local_delta_T ici mais sur le global_delta_T après communication
                ...
                if (global_delta_T < 0.1){
                    ...
                    if (rank == 0){
                        ...
                        fprintf(stderr, "\nRésultats obtenus pour %d processus (MPI-synchrone) en mode %s\n\n", size, mode);
                    }
                }
                }
        }
